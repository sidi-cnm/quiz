...
try:
    import quiz
    print(f"   ✓ VizStyle version {quiz.__version__} importée avec succès!")
...
try:
    ...
    x = [1, 2, 3, 4, 5]
    y = [2, 4, 3, 5, 7]
    fig, ax = quiz.styled_line(x, y, title="Test Line", show=False)
    print("   ✓ styled_line fonctionne")
    
    # Test styled_scatter
    fig, ax = quiz.styled_scatter(x, y, title="Test Scatter", show=False)
    print("   ✓ styled_scatter fonctionne")
    
    # Test styled_bar
    fig, ax = quiz.styled_bar(['A', 'B', 'C'], [10, 20, 15], title="Test Bar", show=False)
    print("   ✓ styled_bar fonctionne")
    
    # Test styled_histogram
    data = np.random.normal(0, 1, 100)
    fig, ax = quiz.styled_histogram(data, title="Test Histogram", show=False)
    print("   ✓ styled_histogram fonctionne")
    
    # Test styled_heatmap
    matrix = np.random.rand(5, 5)
    fig, ax = quiz.styled_heatmap(matrix, title="Test Heatmap", show=False)
    print("   ✓ styled_heatmap fonctionne")
    
    # Test styled_box
    data = [np.random.randn(50), np.random.randn(50)]
    fig, ax = quiz.styled_box(data, title="Test Box", show=False)
...
try:
    ...
    y1 = np.sin(x)
    y2 = np.cos(x)
    fig, ax = quiz.styled_line(x, [y1, y2], 
